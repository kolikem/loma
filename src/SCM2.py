...
def make_cs(cons_start, cons_end, readSu, reads, qs, qs_pos, names):
    ...
    for i in range(cons_start, cons_end+1):     #前後XX塩基分は切り捨てる
        #print(str(i)+"塩基目")
        #print("qs_pos:", qs_pos)
        #print("reads i", reads[0][i], reads[1][i], reads[2][i], reads[3][i], reads[4][i])
        base_cnt += 1
    ...
    out4_name = out4_name.replace("out8", "out4")
    ...
    with open(sys.argv[6].replace("out4","out4a"), mode="w") as f:
        f.write(base_info)
